...
Sarup-A-K
Sarup-A-K
Revise README for ROS 2 Jazzy updates
20518f3
 · 
9 minutes ago
...
Result: turtlebot3, turtlebot3_msgs, and turtlebot3_simulations are now in ~/ros2_ws/src.
    ...
    Shortcut for a lab full of machines: Jazzy binaries exist, so sudo apt install ros-jazzy-turtlebot3 ros-jazzy-turtlebot3-msgs ros-jazzy-turtlebot3-gazebo skips the clone-and-build entirely. Worth checking with apt search ros-jazzy-turtlebot3 before the session — it saves students a 10-minute build.
...
Three things changed from the Humble version:
    ...
    Twist → TwistStamped (velocity now lives under msg.twist, plus a header).
    Publishing once inside __init__ never worked reliably — the publisher has not finished discovery yet, so the single message is dropped. Use a timer.
    linear.x = 1.0 exceeds the burger's maximum of 0.22 m/s; it gets clamped and the circle is not the radius you advertise.
